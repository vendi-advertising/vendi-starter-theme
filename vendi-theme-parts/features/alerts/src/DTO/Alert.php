...
class Alert {
    ...
    public static function createFromPost( $post ): self {
        ...
        return new self(
            ...
            copy: get_field( 'copy', $post ),
            type: $type,
            startDate: new DateTimeImmutable( get_field( 'start_date', $post ) ),
            endDate: new DateTimeImmutable( get_field( 'end_date', $post ) ),
            version: (int) get_field( 'alert_version', $post ),
            dismissible: (string) get_field( 'dismissible', $post ) === 'yes',
