...
add_filter(
    ...
    static function ($template) {
        if (get_query_var(VENDI_QUERY_STRING_KEY_SSO)) {
            ...
            return VENDI_CUSTOM_THEME_PATH . VENDI_CUSTOM_THEME_FEATURE_FOLDER_NAME . '/sso/sso.router.php';
        }
    ...
    static function () {
        ...
        foreach ($sso_providers as $provider) {
            if ('azure_provider' === $provider['acf_fc_layout']) {
                $providerImages['azure'] = vendi_get_svg(VENDI_CUSTOM_THEME_FEATURE_FOLDER_NAME . '/sso/images/small/azure.svg', echo: false);
            } elseif ('github_provider' === $provider['acf_fc_layout']) {
                $providerImages['github'] = vendi_get_svg(VENDI_CUSTOM_THEME_FEATURE_FOLDER_NAME . '/sso/images/small/github.svg', echo: false);
            } elseif ('google_provider' === $provider['acf_fc_layout']) {
                $providerImages['google'] = vendi_get_svg(VENDI_CUSTOM_THEME_FEATURE_FOLDER_NAME . '/sso/images/small/google.svg', echo: false);
            }
        }
